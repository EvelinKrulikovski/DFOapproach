...
%
format long;
 x0 = 2*rand(7,1);  % Not realy needed if parameter list = 3

low=[-10 -10 -10 -10 0.2  0.2 1]'; 
%low=[1.0 0.4 0.6 0.4 0.2  0.2 1]';  % best one 
%up=[4 2 2 2 5 5 5]'; % best one
up=[5 5 5 5 5 5 5]';

[glods_profile,Plist,flist,alfa,radius,func_eval] = glods('func_f1',[],x0,low,up);
...
A(1,1)=Plist(1,1); A(1,2)=Plist(2,1); A(2,4)=Plist(3,1); A(3,3)=Plist(4,1);

% Nz variables: x5 = (1,3), x6 = (2,3), x7= (4,4).
A(1,3)=Plist(5,1); A(2,3)=Plist(6,1); A(4,4)=Plist(7,1);

% A is symmetric then
A(2,1)=Plist(2,1); A(4,2)=Plist(3,1);
A(3,1)=Plist(5,1); A(3,2)=Plist(6,1); 
